...
# 1. Configuração do API Gateway com CORS Global
...
# 3. Integração com a Lambda definindo explicitamente payload_format_version = "2.0"
...
# 4. Rota Curinga para aceitar QUALQUER método e caminho (POST /login, GET /linhas, etc.)
...
# 5. Permissão de Invocação pelo API Gateway
...
resource "aws_lambda_permission" "api_mw" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.api_lambda.function_name
  principal     = "apigateway.amazonaws.com"
...
output "url_da_api" {
  value = "${aws_apigatewayv2_api.http_api.api_endpoint}/diagnostico"
}